...
This is the most complex header and defines a Content Security Policy (CSP). A CSP restricts where the browser can load resources (such as fonts, stylesheets, scripts) from. For the terminal design of this page, inline scripts are explicitly allowed (`unsafe-inline`), although these only run locally. In addition, external resources from trusted sources such as `cdnjs.cloudflare.com` (fonts/icons) and `cdn.jsdelivr.net` (for mathematical formulas via MathJax) are permitted. All design elements and Easter eggs have been optimized to function without blocked inline styles.
