...
Additionally, I use a small client-side JavaScript to personalize the design of this website (terminal look). This script detects the browser you are using and the hostname to display the header in terminal style (e.g., `[Firefox@do3eet.pages.dev /var/www]#`). For improved user experience of the CLI-Navigator, a command history is managed locally in your browser's memory or `sessionStorage`. This allows you to scroll through previously entered commands using the arrow keys. Running processes (such as the `ping` command) can be canceled at any time using the `Ctrl+C` key combination.

For reasons of data economy and data protection ("firewall"), the `ping` command only allows requests to this website itself. All other domains are blocked to prevent your browser from establishing connections to third-party servers without being asked. This data is **not** transmitted to me or third parties and remains exclusively local on your device. You can delete this history yourself at any time by entering the `history -c` command in the terminal or by closing the browser tab.
...
# PeerTube Embedding
Videos from the PeerTube platform are embedded on this website. PeerTube is a decentralized video platform based on the peer-to-peer principle. This means that videos can normally be loaded not only from a server but also from other viewers. The P2P part (sharing the video through your browser) is deactivated for these embeds to the best of my knowledge and belief. However, the PeerTube server operator will still see your IP and browser data.
