...
Zusätzlich verwende ich ein kleines clientseitiges JavaScript, um das Design dieser Website (Terminal-Optik) zu personalisieren. Dieses Skript erkennt den von Ihnen verwendeten Browser und den Hostnamen, um den Header im Terminal-Stil anzuzeigen (z.B. `[Firefox@do3eet.pages.dev /var/www]#`). Für eine verbesserte Benutzerfreundlichkeit des CLI-Navigators wird eine Befehlshistorie lokal im Arbeitsspeicher oder der `sessionStorage` Ihres Browsers verwaltet. Dies ermöglicht es Ihnen, mit den Pfeiltasten durch zuvor eingegebene Befehle zu blättern. Laufende Prozesse (wie der `ping`-Befehl) können jederzeit mit der Tastenkombination `Strg+C` abgebrochen werden. 

Aus Gründen der Datensparsamkeit und des Datenschutzes ("Firewall") erlaubt der `ping`-Befehl nur Anfragen an diese Website selbst. Alle anderen Domains werden blockiert, um zu verhindern, dass Ihr Browser ungefragt Verbindungen zu fremden Servern aufbaut. Diese Daten werden **nicht** an mich oder Dritte übertragen und verbleiben ausschließlich lokal auf Ihrem Endgerät. Sie können diese Historie jederzeit selbst löschen, indem Sie den Befehl `history -c` im Terminal eingeben oder den Browser-Tab schließen.

Ein weiteres kleines Easter-Egg aktiviert sich, wenn die Seite längere Zeit (5 Minuten) ohne Interaktion geöffnet bleibt – als kleine Hommage an alte Monitore erscheint dann ein "Achievement" für einen (natürlich simulierten) Screen Burn-In. Wer ganz nach unten scrollt, findet zudem die "Quelle der Wahrheit". Dabei werden **keine** Daten gespeichert, protokolliert oder an mich oder Dritte übertragen; der Vorgang findet ausschließlich lokal in Ihrem Browser statt – maximal ein "Antiker" schaut Ihnen dabei vielleicht über die Schulter.
